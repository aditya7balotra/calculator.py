...
class calculator(MDApp):
    ...
    def on_start(self):
        ...
        self.root.ids.grid.add_widget(MDFlatButton(text="AC",on_release=self.delete_all,size_hint=[1,1]))
        self.root.ids.grid.add_widget(MDFlatButton(text="del",on_release=self.delete,size_hint=[1,1]))
        self.root.ids.grid.add_widget(MDFlatButton(text="^",on_release=self.to_field,size_hint=[1,1]))

        for i in range(len(opt)):
            self.root.ids.grid.add_widget(MDFlatButton(text=opt[i],on_release=self.to_field_opt,size_hint=[1,1]))
            
        for i in range(10):
            
            self.root.ids.grid.add_widget(MDFlatButton(text=str(i),on_release=self.to_field,size_hint=[1,1]))

        self.root.ids.grid.add_widget(MDFlatButton(text="=",on_release=self.calculate,size_hint=[1,1]))
    ...
    def to_field_opt(self,btn):
        ...
        if self.root.ids.field.text=='undefined':
            ...
            if self.root.ids.field.text[len(self.root.ids.field.text)-1] in opt_check:
